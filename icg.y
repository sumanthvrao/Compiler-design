...
%token IF ELSE WHILE DO FOR CONT BRK RTRN SEMICOLON BINARYOR BINARYAND LT
...
%token<str> ID
%token<num> NUM
%type<node> E T F VarList Expr Extra relExp logExp cond JumpStmt SelectionStmt AssignExpr IterativeStmt
%type<node> FunctionCall Declaration Function Stmt
...
Stmt : Extra SEMICOLON Stmt	{strcpy(tempstr, $1->code); strcat(tempstr, "\n"); strcat(tempstr, $3->code); $$ = createStr(tempstr);
     ...
     | CompoundStmt Stmt	{strcpy(tempstr, $1->code); strcat(tempstr, "\n"); strcat(tempstr, $2->code);$$ = createStr(tempstr);
																 strcpy(tempstr, "");}
	 | IterativeStmt Stmt	{strcpy(tempstr, $1->code); strcat(tempstr, "\n"); strcat(tempstr, $2->code);$$ = createStr(tempstr);
																 strcpy(tempstr, "");}
...
SelectionStmt : IF OB cond CB CompoundStmt	{strcpy(tempstr, "if "); strcat(tempstr, $3->code);
	      ...
	      {strcpy(tempstr, "if "); strcat(tempstr, $3->code);
	       generateLabel(); strcpy(temptemp1, templabel); generateLabel(); strcpy(temptemp2, templabel);
			 ...
			 generateLabel();strcpy(temptemp3, templabel);strcat(tempstr,"goto ");strcat(tempstr,temptemp3);strcat(tempstr,"\n");
			 strcat(tempstr, temptemp2); strcat(tempstr, $7->code);
			 strcat(tempstr, temptemp3);
			 $$ = createStr(tempstr);
			 strcpy(tempstr, "");
			};
IterativeStmt : DO CompoundStmt WHILE OB cond CB SEMICOLON{
											generateLabel(); strcpy(temptemp1, templabel);
											strcpy(tempstr,temptemp1);strcat(tempstr,$2->code);
											strcat(tempstr, "if ");strcat(tempstr, $5->code);strcat(tempstr, " goto ");strcat(tempstr,temptemp1);strcat(tempstr, "\n");
											$$ = createStr(tempstr);
											strcpy(tempstr, "");
										}
	    | WHILE OB cond CB CompoundStmt {
									    	generateLabel(); strcpy(temptemp1, templabel);
											strcpy(tempstr,temptemp1);strcat(tempstr, "ifFalse ");strcat(tempstr, $3->code);
											generateLabel(); strcpy(temptemp2, templabel);
											strcat(tempstr, " goto "); strcat(tempstr, temptemp2); strcat(tempstr, "\n");
											strcat(tempstr, $5->code);
											strcat(tempstr,"goto ");strcat(tempstr,temptemp1);strcat(tempstr,"\n");
											strcat(tempstr, temptemp2);
											$$ = createStr(tempstr);
											strcpy(tempstr, "");
									    }
	      ;
...
VarList : VarList COMMA Call { strcpy(tempstr, $1->code); strcat(tempstr,", "); printf("varlist - %s\n", $1->code);
	...
	| Identifier	{ strcat(tempstr, temp); $$ = createStr(tempstr);
																 strcpy(tempstr, "");}
	;
E : E ADD T	{		if($1->code[0]!='t'&&$3->code[0]!='t')
					...
					{	generateTemp(); ifnum = 0;
						strcpy(tempstr, $1->code); strcat(tempstr, "\n");
					strcat(tempstr, tempT); strcat(tempstr, " = "); strncat(tempstr, $1->code,2);
  					strcat(tempstr, " + "); strcat(tempstr, $3->code);$$ = createStr(tempstr);
																 strcpy(tempstr, "");
					}
					else if($3->code[0]=='t')
					{	generateTemp(); ifnum = 0;
						strcpy(tempstr, $3->code); strcat(tempstr, "\n");
					strcat(tempstr, tempT); strcat(tempstr, " = "); strncat(tempstr, $3->code,2);
  					strcat(tempstr, " + "); strcat(tempstr, $3->code);$$ = createStr(tempstr);
																 strcpy(tempstr, "");
					}
			}
  | E SUB T	{		if($1->code[0]!='t'&&$3->code[0]!='t')
					{	generateTemp(); ifnum = 0;
					strcpy(tempstr, tempT); strcat(tempstr, " = "); strcat(tempstr, $1->code);
  					strcat(tempstr, " - "); strcat(tempstr, $3->code);$$ = createStr(tempstr);
																 strcpy(tempstr, "");
  ...
  | T			{strcpy(tempstr, $1->code);$$ = createStr(tempstr);
																 strcpy(tempstr, "");}
  ;
T : T MUL F	{		if($1->code[0]!='t'&&$3->code[0]!='t')
					{	generateTemp(); ifnum = 0;
					strcpy(tempstr, tempT); strcat(tempstr, " = "); strcat(tempstr, $1->code);
  					strcat(tempstr, " * "); strcat(tempstr, $3->code);$$ = createStr(tempstr);
																 strcpy(tempstr, "");
  ...
  | T DIV F	{		if($1->code[0]!='t'&&$3->code[0]!='t')
					{	generateTemp(); ifnum = 0;
					strcpy(tempstr, tempT); strcat(tempstr, " = "); strcat(tempstr, $1->code);
  					strcat(tempstr, " / "); strcat(tempstr, $3->code);$$ = createStr(tempstr);
																 strcpy(tempstr, "");
